...
def run_pipeline(seen_path=None, near_miss_path=None):
    """
    Full MTU Aero Engines pipeline: fetch → filter → dedup → alert → persist.

    MTU uses a custom HTML portal at mtu.de/careers/online-job-market/.
    All jobs are on a single page (no pagination). Only en_US locale cards
    are scraped.
    ...
    print("[mtu] ── MTU Aero Engines pipeline starting ──")
    print(f"[mtu] Config: seen_path={seen_path.name}, near_miss_path={near_miss_path.name}")

    # ── 1. Fetch ─────────────────────────────────────────────────────────────
    raw_jobs = mtu_fetcher.fetch_jobs()
    total_fetched = len(raw_jobs)
    print(f"[mtu] Fetched {total_fetched} unique listings")

    # ── 2. Filter through 3-gate matcher ────────────────────────────────────
    matched, near_misses = filter_jobs(raw_jobs, mtu_fetcher, config=config)
...
if __name__ == "__main__":
    try:
        run_pipeline()
        notifier.reset_failure_count("mtu")
    except Exception as exc:
        print(f"[mtu] PIPELINE ERROR (non-fatal to outer scheduler): {exc}")
        notifier.notify_pipeline_error("mtu", exc)
