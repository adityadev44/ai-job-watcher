...
SEEN_PATH = ROOT / "seen_jobs_wlfc.json"
NEAR_MISS_PATH = ROOT / "near_misses_wlfc.json"
...
def run_pipeline(seen_path=None, near_miss_path=None):
    """
    Full Willis Lease Finance pipeline: fetch -> filter -> dedup -> alert -> persist.
    ...
    print("[wlfc] ── Willis Lease Finance pipeline starting ──")
    print(f"[wlfc] Config: seen_path={seen_path.name}, near_miss_path={near_miss_path.name}")

    # ── 1. Fetch ─────────────────────────────────────────────────────────────
    raw_jobs = wlfc_fetcher.fetch_jobs()
    total_fetched = len(raw_jobs)
    print(f"[wlfc] Fetched {total_fetched} unique listings")

    # ── 2. Filter through 3-gate matcher ────────────────────────────────────
    matched, near_misses = filter_jobs(raw_jobs, wlfc_fetcher, config=config)
    ...
    alert_sent = False
    if new_matches:
        print(f"[wlfc] {len(new_matches)} new match(es) — sending alert")
        ...
        print("[wlfc] No new matches — nothing to alert")
    ...
    if near_misses:
        ...
        print(f"[wlfc] {len(near_misses)} near-miss(es) appended to {near_miss_path.name}")

    # ── 6. Run summary ───────────────────────────────────────────────────────
    print()
    print("[wlfc] ── Run summary ──────────────────────────────")
    print(f"[wlfc]  Total fetched     : {total_fetched}")
    print(f"[wlfc]  Passed Gate 1     : {g1_pass}  (title family match)")
    print(f"[wlfc]  Passed Gate 3     : {g3_pass}  (exclude filter clear)")
    print(f"[wlfc]  Passed Gate 2     : {total_matched}  (engine domain match)")
    print(f"[wlfc]  New (not seen)    : {len(new_matches)}")
    print(f"[wlfc]  Alert sent        : {'YES' if alert_sent else 'no'}")
    print("[wlfc] ────────────────────────────────────────────")
...
if __name__ == "__main__":
    try:
        run_pipeline()
        notifier.reset_failure_count("wlfc")
    except Exception as exc:
        print(f"[wlfc] PIPELINE ERROR (non-fatal to outer scheduler): {exc}")
        notifier.notify_pipeline_error("wlfc", exc)
