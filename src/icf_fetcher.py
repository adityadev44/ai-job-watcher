...
# ICF International careers — Phenom People ATS
# Portal:     https://careers.icf.com/us/en
# refNum:     IIIIIIUS  (confirmed from CareerConnectResources path in page source)
# Locale:     us/en     (not global/en — ICF is US-headquartered)
# Bootstrap:  GET /us/en/search-results → CSRF token + pageId/pageName from phApp JS config
# Widget API: POST /widgets with ddoKey:"refineSearch" (list) or "jobDetail" (description)
# Note:       ICF covers many practices (energy, health, defense, aviation, digital).
#             All jobs are returned; Gate 1-4 filter to aviation MRO consulting roles.
#             Aviation consulting roles (MRO strategy, fleet analysis, EASA/FAA compliance)
#             will hit domain_terms + engine_specific_terms in Gate 2 when relevant.
# Note:       If 403 errors appear in production, switch to curl_cffi with
#             impersonate="chrome120" (same fix as RTX/careers.rtx.com).

BASE_URL = "https://careers.icf.com"
SEARCH_PAGE = "/us/en/search-results"
REF_NUM = "IIIIIIUS"
PAGE_SIZE = 20
...
def _bootstrap() -> None:
    """
    Bootstrap a requests session from the ICF search-results page.
    ...
    print(f"[icf] Bootstrap OK — pageId={_page_id!r}, pageName={_page_name!r}, csrf={'<found>' if csrf else '<empty>'}")
    ...
    for attempt in range(max_retries + 1):
        ...
        if resp.status_code == 429:
            if attempt < max_retries:
                ...
                print(f"[icf] 429 rate-limit — waiting {wait:.0f}s (attempt {attempt+1}/{max_retries})")
                time.sleep(wait)
                continue
...
def _build_job(raw: dict) -> dict:
    ...
    # Try businessUnit first (multi-brand portals), then company/companyName
    company = (
    ...
    return {
        ...
        "url": f"{BASE_URL}/us/en/job/{req_id}",
        "company": company,
        "source": "icf",
    }
...
def fetch_jobs(max_listings: int = 200, inter_page_delay: float = 0.2) -> list:
    """
    Fetch ICF International job listings via Phenom People widget API.

    ICF covers many practices — all jobs returned and Gate 1-4 filter downstream
    to aviation MRO consulting roles. Uses tail-end pagination to retrieve the newest
    max_listings jobs (Phenom on some tenants returns oldest-first; this handles both).

    Returns a deduplicated list of job dicts.
    """
    _bootstrap()

    # Probe to get total job count before committing to full pagination.
    try:
        probe = _refine_search(from_=0, size=1)
    except RateLimitError:
        raise
    except Exception as exc:
        print(f"[icf] Probe call failed: {exc}")
        return []

    total_avail = probe.get("refineSearch", {}).get("totalHits", 0)
    if total_avail == 0:
        print("[icf] No jobs found — check API connectivity")
        return []

    # Fetch from the tail so we always get the newest jobs, not oldest.
    start_offset = max(0, total_avail - max_listings)
    print(f"[icf] totalHits={total_avail}, fetching from offset {start_offset} (newest {max_listings})")
    ...
    while from_ < total_avail:
        ...
        try:
            ...
            print(f"[icf] Fetch error at from={from_}: {exc}")
            break
        ...
        if not jobs_raw:
            print(f"[icf]   from={from_}: no results — stopping pagination")
            break
        ...
        print(f"[icf]   from={from_}: {len(jobs_raw)} results, {new} new (server total: {total_avail})")

        from_ += len(jobs_raw)
        if inter_page_delay > 0 and from_ < total_avail:
            time.sleep(inter_page_delay)

    print(f"[icf] Total unique jobs fetched: {len(all_jobs)}")
    return all_jobs


def fetch_job_description(application_url: str) -> tuple[str, str]:
    """
    Fetch the full description for an ICF job via the Phenom jobDetail widget API.
    ...
    try:
        ...
        print(f"[icf] Description fetch failed ({req_id}): {exc}")
        return ("", "")
