...
# Gulf Air Group careers portal — same Sniperhire ATS as Sanad (confirmed via
# identical div.row.jobdetail / searchcaption / sectn markup), hosted under
# gulfairgroup.sniperhire.net rather than the gulfair.com domain.

BASE_URL = "https://gulfairgroup.sniperhire.net"
_BROWSER_UA = (
...
def _parse_date(date_str):
    """Convert DD-Mon-YYYY (e.g. '30-Jun-2026') to YYYY-MM-DD. Returns '' on failure."""
    if not date_str:
        return ""
    try:
...
def _parse_listing_page(html):
    ...
    for card in soup.find_all("div", class_="row jobdetail mb-4 ms-0"):
        ...
        company = fields.get("Company", "Gulf Air Group")
        location = fields.get("Location", "Bahrain")
...
def fetch_jobs(max_listings=200, inter_page_delay=0.3):
    """
    Fetch all open vacancies from gulfairgroup.sniperhire.net.

    Paginates via /?pg=0, /?pg=1, ... (0-indexed). No server-side keyword
    filtering — all jobs across Gulf Air Group entities returned; filter
    locally through the 4-gate matcher.
    """
    ...
    while len(all_jobs) < max_listings:
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    print(f"[gulf_air] Page {page}: request failed: {exc}")
                    return all_jobs
                time.sleep(2 ** (attempt + 1))

        if resp is None or resp.status_code != 200:
            print(f"[gulf_air] Page {page}: HTTP {getattr(resp, 'status_code', '?')} — stopping")
            break
        ...
        print(f"[gulf_air] Page {page}: {len(page_jobs)} jobs, {len(new_jobs)} new — total: {len(all_jobs)}")
    ...
    try:
        ...
        if categs:
            for li in categs.find_all("li"):
                ...
                value_div = li.find("div", class_="sc-val")
