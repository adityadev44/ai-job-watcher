...
NEAR_MISS_PATH = ROOT / "near_misses_elfc.json"
...
def run_pipeline(seen_path=None, near_miss_path=None):
    ...
    print(f"[elfc] Config: seen_path={seen_path.name}, near_miss_path={near_miss_path.name}")
    ...
    if near_misses:
        ...
        print(f"[elfc] {len(near_misses)} near-miss(es) appended to {near_miss_path.name}")
