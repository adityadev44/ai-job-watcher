...
SEEN_PATH = ROOT / "seen_jobs_aercap.json"
NEAR_MISS_PATH = ROOT / "near_misses_aercap.json"
...
def run_pipeline(seen_path=None, near_miss_path=None):
    """
    Full AerCap pipeline: fetch → filter → dedup → alert → persist.
    ...
    print("[aercap] ── AerCap pipeline starting ──")
    print(f"[aercap] Config: seen_path={seen_path.name}, near_miss_path={near_miss_path.name}")

    # ── 1. Fetch ─────────────────────────────────────────────────────────────
    raw_jobs = aercap_fetcher.fetch_jobs()
    total_fetched = len(raw_jobs)
    print(f"[aercap] Fetched {total_fetched} unique listings")

    # ── 2. Filter through 3-gate matcher ────────────────────────────────────
    matched, near_misses = filter_jobs(raw_jobs, aercap_fetcher, config=config)
    ...
    alert_sent = False
    if new_matches:
        print(f"[aercap] {len(new_matches)} new match(es) — sending alert")
        ...
        print("[aercap] No new matches — nothing to alert")
    ...
    if near_misses:
        ...
        print(f"[aercap] {len(near_misses)} near-miss(es) appended to {near_miss_path.name}")

    # ── 6. Run summary ───────────────────────────────────────────────────────
    print()
    print("[aercap] ── Run summary ──────────────────────────────")
    print(f"[aercap]  Total fetched     : {total_fetched}")
    print(f"[aercap]  Passed Gate 1     : {g1_pass}  (title family match)")
    print(f"[aercap]  Passed Gate 3     : {g3_pass}  (exclude filter clear)")
    print(f"[aercap]  Passed Gate 2     : {total_matched}  (engine domain match)")
    print(f"[aercap]  New (not seen)    : {len(new_matches)}")
    print(f"[aercap]  Alert sent        : {'YES' if alert_sent else 'no'}")
    print("[aercap] ────────────────────────────────────────────")
...
if __name__ == "__main__":
    try:
        run_pipeline()
        notifier.reset_failure_count("aercap")
    except Exception as exc:
        print(f"[aercap] PIPELINE ERROR (non-fatal to outer scheduler): {exc}")
        notifier.notify_pipeline_error("aercap", exc)
