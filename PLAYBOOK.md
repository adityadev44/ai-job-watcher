...
| MTU Maintenance | Custom TYPO3 CMS (`mtu.de/careers`) | HTML scraping (requests + BeautifulSoup) | `run_mtu.py` | `GET https://www.mtu.de/careers/online-job-market/s/all/all/all/professionals/` — all professional-level jobs across all MTU entities on a single page, no pagination. Job cards: `<a href="/careers/online-job-market/job-posting/{slug}/">` → `<h3>title</h3><ul><li>company</li><li>location</li></ul>`. **Pre-filtered to MTU Maintenance entities** in `run_mtu.py` (company contains "MTU Maintenance") — excludes MTU Aero Engines AG manufacturing jobs. Subsidiaries: Hannover (CFM56/V2500/CF6/GP7200/PW800), Berlin-Brandenburg (GE90/LEAP-1A\/1B/CFM56), Canada (Delta BC), Fort Worth TX, Australia, Serbia. **No posting date available anywhere on the portal** — `posting_date=""` → shows "N/A" in alerts. Description: GET `{BASE}/careers/online-job-market/job-posting/{slug}/` — strip nav/header/footer, extract from `<h2>` onwards. German-language titles (majority of Hannover/Berlin-Brandenburg roles) fail Gate 1 naturally — English titles from Canada, Fort Worth, Australia, and international leadership roles will pass (same limitation as LHT German branches). |
...
| ✅ 9 | MTU Maintenance | Custom TYPO3 CMS (plain HTML; single page, no pagination; pre-filter to MTU Maintenance; no posting date; German titles fail Gate 1 naturally) | Germany/Canada/Serbia/Fort Worth/Australia |
